...
# New Feature (Maybe Coming :)
@bot.on_message(filters.me & filters.command(["get_reac"]))
def get_reactions(Client, Message):
    # tmp = raw.functions.messages.GetMessageReactionsList(peer=int(Message.chat.id), id=int(Message.reply_to_message_id), limit=10)
    tmp = raw.functions.messages.GetMessageReactionsList(peer=int(Message.chat.id), id=int(Message.reply_to_message_id), limit=10)
    # reacts = {}
    # for message in bot.search_messages(Message.chat.id, from_user="me"):
    #     if hasattr(message, 'reactions'):
    #         reactions = bot.get_messages(Message.chat.id,Message.reply_to_message_id).reactions
    #         reacts[f'ID: [{message}](tg://user?id={message})']
    # Message.edit_text(reactions)
